...
func Test_E2E_Response_Exec_PrintingEnv(t *testing.T) {
	RunTestWithEnv(
		...
		LineEquals(1, `MOCK_BASE_API=`),
		LineEquals(2, `MOCK_HOST=localhost:{{TEST_E2E_PORT}}`),
		LineRegexMatches(3, `MOCK_REQUEST_BODY=.*`),
		LineEquals(4, `MOCK_REQUEST_ENDPOINT=with/exec/print/env/with/param/bar`),
		LineRegexMatches(5, `MOCK_REQUEST_HEADERS=.*`),
		LineEquals(6, `MOCK_REQUEST_HEADER_HEADER_ONE=Header value one`),
		LineEquals(7, `MOCK_REQUEST_HEADER_HEADER_TWO=Header value two`),
		LineEquals(8, `MOCK_REQUEST_HOST=localhost:{{TEST_E2E_PORT}}`),
		LineEquals(9, `MOCK_REQUEST_HTTPS=false`),
		LineEquals(10, `MOCK_REQUEST_METHOD=get`),
		LineEquals(11, `MOCK_REQUEST_NTH=1`),
		LineEquals(12, `MOCK_REQUEST_QUERYSTRING=foo=bar`),
		LineEquals(13, `MOCK_REQUEST_QUERYSTRING_FOO=bar`),
		LineEquals(14, `MOCK_REQUEST_URL=http://localhost:{{TEST_E2E_PORT}}/with/exec/print/env/with/param/bar`),
		LineRegexMatches(15, `MOCK_RESPONSE_BODY=.*`),
		LineRegexMatches(16, `MOCK_RESPONSE_HEADERS=.*`),
		LineRegexMatches(17, `MOCK_RESPONSE_STATUS_CODE=.*`),
		LineEquals(18, `MOCK_ROUTE_PARAM_FOO=bar`),
	)
}
