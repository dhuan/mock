...
func Test_E2E_Response_Exec_PrintingEnv(t *testing.T) {
	RunTestWithEnv(
		...
		LineEquals(9, `MOCK_REQUEST_METHOD=get`),
		LineEquals(10, `MOCK_REQUEST_NTH=1`),
		LineEquals(11, `MOCK_REQUEST_QUERYSTRING=foo=bar`),
		LineEquals(12, `MOCK_REQUEST_QUERYSTRING_FOO=bar`),
		LineEquals(13, `MOCK_REQUEST_URL=http://localhost:{{TEST_E2E_PORT}}/with/exec/print/env/with/param/bar`),
		LineRegexMatches(14, `MOCK_RESPONSE_BODY=.*`),
		LineRegexMatches(15, `MOCK_RESPONSE_HEADERS=.*`),
		LineRegexMatches(16, `MOCK_RESPONSE_STATUS_CODE=.*`),
		LineEquals(17, `MOCK_ROUTE_PARAM_FOO=bar`),
	)
}
